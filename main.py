...
DB_FILE = BASE_DIR / "vibify.db"
LEGACY_LIBRARY_FILE = BASE_DIR / "library.json"

SESSION_COOKIE = "vibify_session"
GUEST_COOKIE = "vibify_guest_id"
SESSION_DAYS = 30
GUEST_FREE_TOKENS = 2
...
def init_database():
    ...
    with db_connect() as db:
        ...
        db.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_videos_user_id_saved_at
            ON videos(user_id, saved_at DESC);
...
def set_session_cookie(
    ...
    response.set_cookie(
        ...
        httponly=True,
        samesite="lax",
        secure=False,  # Set True when Vibify is served over HTTPS.
        path="/",
    )
...
def require_user(
    ...
    if user is None:
        ...
        raise HTTPException(
            status_code=401,
            detail=(
                "Please log in or sign up to use Library."
            ),
        )
...
def get_guest_tokens_remaining(
    guest_id: str,
) -> int:
    ...
    return max(
        0,
        GUEST_FREE_TOKENS - used,
    )


def consume_guest_token(
    guest_id: str,
) -> int:
    ...
    with db_connect() as db:
        ...
        if used >= GUEST_FREE_TOKENS:

            raise HTTPException(
                status_code=403,
                detail=(
                    "You have used your 2 free guest generations. "
    ...
    return max(
        0,
        GUEST_FREE_TOKENS - new_used,
    )


def video_row_to_dict(
    row: sqlite3.Row,
) -> dict:
...
def auth_guest(
    ...
    remaining = get_guest_tokens_remaining(
        guest_id
    )
    ...
    return {
        "success": True,
        "is_guest": True,
        "guest_tokens_total": GUEST_FREE_TOKENS,
        "guest_tokens_remaining": remaining,
...
def google_auth_callback(
    ...
    with db_connect() as db:
        ...
        if state_row is None:
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid or expired Google login state.",
            )
    ...
    if not google_sub or not email:
        ...
        raise HTTPException(
            status_code=502,
            detail="Google did not return a usable account.",
        )
...
def build_minimax_prompt(
    ...
    scene_transition_instruction = ""

    if shot_index > 0:

        scene_transition_instruction = f"""
SCENE TRANSITION:

The input image is the final frame of the previous shot.

Preserve the product exactly from the input image.
...
However, DO NOT preserve the previous shot's background or environment.

Replace the previous background completely with:

{background}

The new requested environment must become clearly visible from the
beginning of this shot.

Treat the previous frame only as a continuity reference for the product,
not as a requirement to keep the previous environment.

The product should transition naturally into the new environment while
remaining visually identical.

Do not blend the old environment with the new environment unless needed
for a very brief natural transition.
"""
...
Preserve the product's visual identity, but do not preserve the original
image's crop or subject scale.

Recompose the shot so the complete product is comfortably visible inside
the frame.

The product should occupy approximately 35% to 50% of the frame,
leaving clear visible space around it for the environment.

If the uploaded reference image is tightly cropped or the product appears
very large, smoothly pull the virtual camera backward and create a wider
composition.

Do not enlarge the product simply because it appears large in the
reference image.

Maintain realistic product proportions while allowing the camera distance
and framing to change.

{scene_transition_instruction}

SCENE:

The required environment for this shot is:

{background}

Make the requested environment clearly recognizable.

Do not substitute it with the environment from a previous shot.
...
AI-ENHANCED CREATIVE DIRECTION:
...
The product must remain the primary visual subject without occupying
...
def generate(
    ...
    if current_user is None:
        ...
        guest_id, guest_cookie_is_new = (
            get_or_create_guest_id(request)
        )

        if get_guest_tokens_remaining(guest_id) <= 0:
    ...
    guest_tokens_remaining = None

    if current_user is None:

        guest_tokens_remaining = (
            consume_guest_token(guest_id)
        )

        if guest_cookie_is_new:
            set_guest_cookie(
                response,
                guest_id,
            )
...
# =============================================================================
# SAVE VIDEO TO LIBRARY — LOGGED-IN USERS ONLY
# =============================================================================
    ...
    with db_connect() as db:
        ...
        if existing is not None:
            ...
            return {
                "success": True,
                "already_saved": True,
                "video": video_row_to_dict(existing),
            }

        library_video_id = uuid.uuid4().hex
...
# =============================================================================
# GET SAVED LIBRARY VIDEOS — CURRENT USER ONLY
# =============================================================================
...
# =============================================================================
# DELETE GENERATED VIDEO — OWNER ONLY
# =============================================================================
